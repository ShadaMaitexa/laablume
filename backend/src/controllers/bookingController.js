...
const PLATFORM_FEE = 50; // ₹50 platform fee for lab bookings
const DOCTOR_PLATFORM_FEE = 15; // ₹15 platform fee for doctor bookings
...
const createBooking = async (req, res) => {
    ...
    try {
        ...
        if (bookingType === 'test' || !bookingType) {
            ...
            // Doctor booking: Total - ₹15 = Hospital/Doctor payout, ₹15 = Admin revenue
            platformFee = DOCTOR_PLATFORM_FEE;
            const hospitalAmount = totalAmount - platformFee;
            revenue = {
                hospitalAmount: hospitalAmount > 0 ? hospitalAmount : 0,
                adminAmount: platformFee
            };
        }
...
const updateVisitSummary = async (req, res) => {
    try {
        ...
        booking.completedAt = Date.now(); // Set completion timestamp for chat TTL
